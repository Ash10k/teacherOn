...
public class saveStep4Repo {

    private AuthAPI sendData;
    private MutableLiveData<ErrorData> errorMessage = new MutableLiveData<>();

    public saveStep4Repo() {
        sendData = RetrofitBuilder.build().create(AuthAPI.class);
    }

    public LiveData<saveResponse> saveData(List<step4teacher.Degree> subjects, String userId, String token) {
        MutableLiveData<saveResponse> userSessionLiveData = new MutableLiveData<>();
        Call<saveResponse> myCall = sendData.saveStep4(token,new step4teacher.DegreeRequestBody(subjects, userId));
