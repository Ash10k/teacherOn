...
public class EditPost extends AppCompatActivity {
    ...
    String  budgetCurrencyId,budgetType,communicateLanguageId,genderPreference,
            levelId,location,requirementType,requirements,travelLimit,tutorFromCountryId,
            tutorOption,tutorType,id,suserId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPrefLocal sharedPrefLocal = new SharedPrefLocal(EditPost.this);
        ...
        savestep2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (budget.isEmpty()) {
                    Toast.makeText(EditPost.this, "Enter Amount", Toast.LENGTH_SHORT).show();
                } else if (travel_limit.isEmpty()) {
                    Toast.makeText(EditPost.this, "Enter Travel KM", Toast.LENGTH_SHORT).show();
                } else {
                    step2VModelStudent viewModel = new ViewModelProvider(EditPost.this).get(step2VModelStudent.class);
                    networkLoader.showLoadingDialog(EditPost.this);
                    ...
                    viewModel.editPost(subject_id, from_level_id, user_id, requirement_type, tutor_option, travel_limit, budget, budget_type, gender_preference, tutor_type, budget_currency_id, communicate_language_id, tutor_from_country_id, details, token,lo,id).observe(EditPost.this, new Observer<passwordResponse>() {
                        @Override
                        public void onChanged(passwordResponse loginResponse) {
                            if (loginResponse != null) {
                               ...
                               /* SharedPrefLocal sharedPrefLocal = new SharedPrefLocal(EditPost.this);
                                ...
                                Intent intent=new Intent(EditPost.this,BottomNav.class);
                                startActivity(intent);
                                */
                                Toast.makeText(EditPost.this, "" + loginResponse.message, Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                // Handle null response here if needed
                                Toast.makeText(EditPost.this, SERVER_ERROR, Toast.LENGTH_SHORT).show();
                            }
                    ...
                    viewModel.getErrorMessage().observe(EditPost.this, new Observer<ErrorData>() {
                        @Override
        ...
        if (ContextCompat.checkSelfPermission(EditPost.this, android.Manifest.permission.CAMERA) == PackageManager.PERMISSION_DENIED) {
            Toast.makeText(EditPost.this, "Please give permission", Toast.LENGTH_SHORT).show();
            ActivityCompat.requestPermissions(EditPost.this, new String[]{Manifest.permission.CAMERA, MANAGE_EXTERNAL_STORAGE, WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, ACCESS_COARSE_LOCATION, ACCESS_FINE_LOCATION}, 101);
        ...
        budget =  (intent.getStringExtra("budget"));
          budgetCurrencyId = intent.getStringExtra("budget_currency_id");
          budgetType = intent.getStringExtra("budget_type");
          communicateLanguageId = intent.getStringExtra("communicate_language_id");
          genderPreference = intent.getStringExtra("gender_preference");
          levelId = intent.getStringExtra("level_id");
          location = intent.getStringExtra("location");
          requirementType = intent.getStringExtra("requirement_type");
          requirements = intent.getStringExtra("requirements");
          travelLimit = String.valueOf(intent.getIntExtra("travel_limit",0));
          tutorFromCountryId = intent.getStringExtra("tutor_from_country_id");
          tutorOption = intent.getStringExtra("tutor_option");
          tutorType = intent.getStringExtra("tutor_type");
          suserId = intent.getStringExtra("user_id");
          id = String.valueOf(intent.getIntExtra("id",0));

          // Toast.makeText(EditPost.this, "received ini:"+budget, Toast.LENGTH_SHORT).show();


        loctiio.setText(location);
        degreeNameInput.setText(travelLimit);
        budgtinp.setText(budget);
        detailsrequi.setText(requirements);
        ...
        try {
            ...
            myCall.enqueue(new Callback<languageResponse>() {
                ...
                public void onResponse(Call<languageResponse> call, Response<languageResponse> response) {
                    ...
                    if (response.isSuccessful()) {
                        if ((response.body().status).equals("success")) {
                            ...
                            Toast.makeText(EditPost.this, "" + response.body().message, Toast.LENGTH_SHORT).show();
                            networkLoader.dismissLoadingDialog();

                        }
                    } else {
                        Toast.makeText(EditPost.this, "Server Error" + response.raw(), Toast.LENGTH_LONG).show();
                ...
                public void onFailure(Call<languageResponse> call, Throwable t) {
                    Toast.makeText(EditPost.this, "Server Error", Toast.LENGTH_SHORT).show();
                    t.printStackTrace();
                    networkLoader.dismissLoadingDialog();
                }
            });
    ...
    private void selectImage(int request_made_by) {
        try {
            ...
            camuri = EditPost.this.getContentResolver()
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == SELECT_PICTURE) {
            //  Toast.makeText(EditPost.this, "called onactivity result", Toast.LENGTH_SHORT).show();
            Uri selectedImageUri = data.getData();
            try {
                bitmap = MediaStore.Images.Media
                        .getBitmap(EditPost.this.getContentResolver(), selectedImageUri);
                ...
                Glide.with(EditPost.this).load(bitmap).apply(options).into(selectedImg);
...
// Get real path from URI
                ...
                CursorLoader cursorLoader = new CursorLoader(EditPost.this, selectedImageUri, projection, null, null, null);
            ...
            // selectImage();
            Toast.makeText(EditPost.this, "Please click image", Toast.LENGTH_SHORT).show();
    ...
    public String getRealPathFromURI(Uri contentURI) {
        String result;
        Cursor cursor = EditPost.this.getContentResolver().query(contentURI, null,
                null, null, null);
        if (cursor == null) {
            // path
        ...
        try {
            ...
            call.enqueue(new Callback<saveResponse>() {
                ...
                public void onFailure(Call<saveResponse> call, Throwable t) {
                    ...
                    networkLoader.dismissLoadingDialog();
                    Toast.makeText(EditPost.this, "Check your internet connection.", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private boolean get_edu_journey() {
        ...
        try {
            ...
            Toast.makeText(EditPost.this, "Some thing wrong", Toast.LENGTH_SHORT).show();
            return false;
        }
    ...
    private void setupSpinners2() {
        ArrayAdapter<String> subjectAdapter = new ArrayAdapter<>(EditPost.this, android.R.layout.simple_spinner_dropdown_item, getSubjectNames2());
        subjectSpinner.setAdapter(subjectAdapter);

        ArrayAdapter<String> levelAdapter = new ArrayAdapter<>(EditPost.this, android.R.layout.simple_spinner_dropdown_item, getLevelNames());
