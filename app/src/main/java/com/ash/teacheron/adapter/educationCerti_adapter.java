...
public class educationCerti_adapter extends ArrayAdapter {
    Context c;
    private List<recommendedTeacherResponse.TutorRequest.TeacherCertification> list;
    ...
    public educationCerti_adapter(Context context, List objects) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (list != null) {
            product_name.setText( (list.get(position).institutionName) +"\n"+list.get(position).degreeName);
            items.setText(String.valueOf(""+list.get(position).degreeType));
