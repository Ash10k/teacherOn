...
public class chat_adapter extends ArrayAdapter<ChatResponseDataModel.ChatUser> implements Filterable {

    Context c;
    List<ChatResponseDataModel.ChatUser> list;
    List<ChatResponseDataModel.ChatUser> filteredList;
    LayoutInflater inflater;

    public chat_adapter(Context context, List<ChatResponseDataModel.ChatUser> objects) {
    ...
    @Override
    public ChatResponseDataModel.ChatUser getItem(int position) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            ChatResponseDataModel.ChatUser currentUser = getItem(position);
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                List<ChatResponseDataModel.ChatUser> filteredResults = new ArrayList<>();
                if (constraint == null || constraint.length() == 0) {
                    filteredResults.addAll(list);
                } else {
                    String filterPattern = constraint.toString().toLowerCase().trim();
                    for (ChatResponseDataModel.ChatUser user : list) {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                filteredList.clear();
                filteredList.addAll((List<ChatResponseDataModel.ChatUser>) results.values);
